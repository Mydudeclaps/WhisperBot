...
> **Status: Historical — already applied.** This patch has been fully merged into WhisperBot (`/lore` runs on SQLite via `database/database.js` and `services/loreService.js`; there is no `mongoose` dependency and no `models/Lore.js` in the current codebase). Kept here as a record of why the change was made, not as an action item.
